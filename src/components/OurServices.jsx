...
function OurServices() {
  return (
    <div className="md:mx-[4rem] mx-4">
      <h5 className="text-orange-500 md:mb-6 mb-3 text-center lg:text-2xl">Our Services</h5>
      ...
      <div className="mt-8">
        ...
        {[ 
          ...
          <div 
            key={index} 
            className="flex flex-col md:flex-row border-b md:p-4 p-4 items-start md:items-center hover:bg-[#f5f4f8] hover:rounded-xl border-b-[#ccc9d2] gap-4 md:gap-8 transition-all duration-300 ease-in-out"
          >
            <p className="text-[#5d5968] text-[14px] md:text-base lg:text-lg text-center md:text-left w-full md:w-[60%] lg:w-[50%]">
              {service.desc}
            </p>
            <h4 className="ml-auto text-[#3a3645] font-bold text-[16px] md:text-xl lg:text-2xl text-center md:text-left w-full md:w-[40%] lg:w-[50%] md:ml-[4em] lg:ml-[10em]">
              {service.title}
            </h4>
          </div>
        ))}
